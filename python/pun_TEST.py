...
class PythonTests:
    ...
    def check_001_cpp_module(self):
        ...
        try:
            ...
            raise TestFailedError('importing \'_pun\' failed: \'{}\''.format(str(e)))

        try:
            import pun
        except ImportError as e:
            raise TestFailedError('importing \'pun\' failed: \'{}\''.format(str(e)))

    def check_002_QualifiedName(self):
        """Check if an instance of QualifiedName can be created."""
        from pun import QualifiedName
    ...
    def check_006_Observable(self):
        ...
        from pun import Observable, Parameters, Kinematics, Options, QualifiedName
    ...
    def check_007_Constraint(self):
        ...
        """
        from pun import Constraint, Options
    ...
    def check_008_Model(self):
        from pun import Model, Parameters, Options
...
class LoggingTests(unittest.TestCase):
    ...
    def test_log_levels(self):
        ...
        import pun, logging, _pun
        from pun import _NativeLogLevel as ll
        levels = [ll.DEBUG, ll.INFO, ll.WARNING, ll.ERROR]

        for set_level in levels:
            _pun._set_native_log_level(set_level)

            for check_level in levels:

                if check_level <= set_level:
                    with self.assertLogs('pun', level='DEBUG') as cm:
                        _pun._emit_native_log("myId", check_level, "msg")
                else:
                    # workaround for old unittest version, w/o assertNoLogs
                    with self.assertRaisesRegex(AssertionError,
                        "no logs of level DEBUG or higher triggered on pun"):
                        with self.assertLogs('pun', level='DEBUG') as cm:
                            _pun._emit_native_log("id", check_level, "msg")

    def test_log_000(self):
        "Computation of specific observable should log error"
        import pun

        with self.assertLogs('pun', level='ERROR') as cm:
            pun.Observable.make(
                    "B->pilnu::BR",
                    pun.Parameters.Defaults(),
                    pun.Kinematics(q2_min=1, q2_max=6),
                    pun.Options(**{'U': 'c'})) # will  be overwritten by the
                                               # implementation of the observable
            self.assertEqual(cm.output,
                    [r"""ERROR:pun:[ConcreteObservableEntry.make] Observable 'B->pilnu::BR' forces option key 'U' to value 'u', overriding user-provided value 'c'"""])
