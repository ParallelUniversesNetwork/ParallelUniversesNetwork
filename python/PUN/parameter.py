...
from _pun import _Parameters, QualifiedName, Unit
import re

class Parameters(_Parameters):
    """
    Represents the set of parameters known to pun.
    ...
    def to_yaml(self, names=None):
        """
        Converts an pun.Parameters object to a YAML representation.
    ...
    def dump(self, file, **kwargs):
        """
        Dumps an pun.Parameters object to a YAML file.
    ...
    def FromWCxf(w):
        """
        Imports Wilson coefficients into an pun.Parameters object from a wilson.Wilson object.

        :param w: WET Wilson coefficients in the pun basis.
        ...
        if wc.basis != 'pun':
            raise ValueError('Wilson coefficients must be passed to this function in the pun basis')

        # Needs to be revisited, once pun support WET-4 or WET-3 bases
        if wc.scale != 4.2:
            raise ValueError('Wilson coefficients must be passed to this function at the reference scale 4.2 GeV')

        # the following coefficients are treated as real-valued in pun
        real_coeffs = [
        ...
        for name, value in wc.dict.items():
            ...
            # Add values provided by WCxf to pun central (SM) values
