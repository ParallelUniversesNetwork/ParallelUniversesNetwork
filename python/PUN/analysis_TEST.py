...
class ClassMethodTests(unittest.TestCase):
    ...
    def test_sanitize_manual_input(self):
        ...
        for case in input_output_cases:
            ...
            self.assertEqual(
                pun.Analysis._sanitize_manual_input(case['dirty']),
                case['clean']
            )
            self.assertEqual(
                type(pun.Analysis._sanitize_manual_input(case['dirty'])),
                type(case['clean'])
            )
            # check Yaml string
            self.assertNotEqual(
            ...
            self.assertEqual(
                yaml.dump(pun.Analysis._sanitize_manual_input(case['dirty'])),
                yaml.dump(case['clean'])
            )
