...
class PMCDataFile(HDF5DataFile):
    def __init__(self, file):
        ...
        if 'format' not in self.file.attrs:
            pun.warn('input file does not have attribute \'format\'; assuming format \'PMC\'')
...
class MCMCDataFile(HDF5DataFile):
    def __init__(self, file):
        ...
        if 'format' not in self.file.attrs:
            pun.warn('input file does not have attribute \'format\'; assuming format \'MCMC\'')
    ...
    def data(self):
        ...
        if 'main run' not in self.file:
            pun.warn('input file does not contain results from a main run')
            groupname = 'prerun'
...
class UncertaintyDataFile(HDF5DataFile):
    def __init__(self, file):
        ...
        # check that the input file has format=PMC
        if 'format' not in self.file.attrs:
            pun.warn('input file does not have attribute \'format\'; assuming format \'UNC\'')
