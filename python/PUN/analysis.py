...
class BestFitPoint:
    """
    Represents the best-fit point of a Bayesian analysis undertaken with the :class:`Analysis <pun.Analysis>` class.
    """
...
class Analysis:
    ...
    :param priors: The priors for this analysis as a list of prior descriptions. See :ref:`below <pun-Analysis-prior-descriptions>` for what consitutes a valid prior description.
    ...
    def __init__(self, priors, likelihood, global_options={}, manual_constraints={}, fixed_parameters={}):
        ...
        self.parameters = pun.Parameters.Defaults()
        self.global_options = pun.Options()
        self._log_likelihood = pun.LogLikelihood(self.parameters)
        self._log_posterior = pun.LogPosterior(self._log_likelihood)
        self.varied_parameters = []
        self.bounds = []

        pun.info('Creating analysis with {nprior} priors, {nconst} pun-wide constraints, {nopts} global options, {nmanual} manually-entered constraints and {nparams} fixed parameters.'.format(
        ...
        pun.debug('priors:')
        for p in priors:
            pun.debug(' - {name} ({type}) [{min}, {max}]'.format(name=p['parameter'], type=p['type'], min=p['min'], max=p['max']))
        pun.debug('constraints:')
        for cn in likelihood:
            pun.debug(' - {name}'.format(name=cn))
        pun.debug('manual_constraints:')
        for cn, ce in manual_constraints.items():
            pun.debug(' - {name}'.format(name=cn))
        pun.debug('fixed_parameters:')
        for pn, pe in fixed_parameters.items():
            pun.debug(' - {name}'.format(name=pn))
        ...
        for prior in priors:
            ...
            if 'uniform' == prior_type or 'flat' == prior_type:
                self._log_posterior.add(pun.LogPrior.Flat(self.parameters, parameter, pun.ParameterRange(minv, maxv)), False)
                ...
                self._log_posterior.add(
                    pun.LogPrior.Gauss(
                        self.parameters, parameter, pun.ParameterRange(minv, maxv),
        ...
        for constraint_name in likelihood:
            constraint = pun.Constraint.make(constraint_name, self.global_options)
        ...
        for constraint_name, constraint_data in manual_constraints.items():
            ...
            constraint_entry = pun.ConstraintEntry.deserialize(constraint_name, yaml_string)
        ...
        if (len(used_but_unvaried) > 0):
            pun.info('likelihood probably depends on {} parameter(s) that do not appear in the prior; check prior?'.format(len(used_but_unvaried)))
        for n in used_but_unvaried:
            pun.debug('used, but not included in any prior: \'{}\''.format(n))
        for n in varied_parameter_names - used_parameter_names:
            pun.warn('likelihood does not depend on parameter \'{}\'; remove from prior or check options!'.format(n))
    ...
    def clone(self):
        """Returns an independent instance of pun.Analysis."""
        return pun.Analysis(**self.init_args)


    def goodness_of_fit(self):
        """Returns a goodness-of-fit summary for the current parameter point."""
        return pun.GoodnessOfFit(self._log_posterior)
    ...
    def optimize(self, start_point=None, rng=np.random.mtrand, **kwargs):
        ...
        :param start_point: Parameter point from which to start the optimization, with the elements in the same order as in pun.Analysis.varied_parameters.
        ...
        if not res.success:
            pun.warn('Optimization did not succeed')
            pun.warn('  optimizer'' message reas: {}'.format(res.message))
        else:
            pun.info('Optimization goal achieved after {nfev} function evaluations'.format(nfev=res.nfev))
        ...
        return pun.BestFitPoint(self, bfp)
    ...
    def log_pdf(self, x, *args):
        ...
        :param x: Parameter point, with the elements in the same order as in pun.Analysis.varied_parameters, rescaled so that every element is in the interval [-1, +1].
        :type x: iterable
    ...
    def sample(self, N=1000, stride=5, pre_N=150, preruns=3, cov_scale=0.1, observables=None, start_point=None, rng=np.random.mtrand):
        ...
        for i in progressbar(range(0, preruns), desc="Pre-runs", leave=False):
            pun.info('Prerun {} out of {}'.format(i, preruns))
            accept_count = sampler.run(pre_N)
            accept_rate  = accept_count / pre_N * 100
            pun.info('Prerun {}: acceptance rate is {:3.0f}%'.format(i, accept_rate))
            sampler.adapt()
        sampler.clear()

        # obtain final samples
        pun.info('Main run: started ...')
        ...
        pun.info('Main run: acceptance rate is {:3.0f}%'.format(accept_rate))
    ...
    def sample_pmc(self, log_proposal, step_N=1000, steps=10, final_N=5000, rng=np.random.mtrand,
        ...
        log(posterior) by other means, e.g., by using :meth:`pun.Analysis.sample`.
        ...
        for step in progressbar(range(steps), desc="Adaptions", leave=False):
            ...
            pun.info('Perplexity of the last samples after sampling in step {}: {}'.format(step, last_perplexity))
            if last_perplexity < 0.05:
                pun.warn("Last step's perplexity is very low. This could possibly be improved by running the markov chains that are used to form the initial PDF for a bit longer")
            ...
            pun.info('Perplexity of all previous samples after sampling in step {}: {}'.format(step, self._perplexity(adjusted_weights)))
            ...
            if last_perplexity > final_perplexity_threshold:
                pun.info('Perplexity threshold reached after {} step(s)'.format(step))
                break
        ...
        pun.info('Perplexity after final samples: {}'.format(perplexity))
