...
class Plotter:
    ...
    Plots can contain pun observables, pun constraints, and :class:`Analysis <pun.Analysis>` results.
    ...
    def _weighted_quantiles(values, quantiles, weights=None):
        ...
        if (np.any(quantiles < 0) or np.any(quantiles > 1)):
            pun.error('Quantiles should be in [0, 1]')
        if (np.any(weights < 0)):
            pun.error('The sample weights cannot be negative')
        if (np.sum(weights) == 0):
            pun.error('The sum of the sample weights evaluated to zero')
        ...
        if 'x' in myplot:
            ...
            if 'scaling_factor' in myx:
                ...
                if 'format' in myx:
                    ...
                    self.xformat = '${x}$'
                    pun.warn("Argument plot:x:format might be required when using plot:x:scale to avoid side effects")
                ...
                if 'format' in myx:
                    pun.warn("Argument plot:x:format is only used when plot:x:scale is used")
        ...
        if 'y' in myplot:
            ...
            if 'scaling_factor' in myy:
                ...
                if 'format' in myy:
                    ...
                    self.yformat = '${x}$'
                    pun.warn("Argument plot:y:format might be required when using plot:y:scale to avoid side effects")
                ...
                if 'format' in myy:
                    pun.warn("Argument plot:y:format is only used when plot:y:scale is used")
    ...
    class Observable(BasePlot):
        """Plots a single pun observable w/o uncertainties as a function of one kinemtic variable or one parameter"""
         ...
         * ``observable`` (:class:`QualifiedName <pun.QualifiedName>`) -- The name of the observable that will be plotted.
           Must identify one of the observables known to pun; see `the complete list of observables <../observables.html>`_.
        ...
        def plot(self):
            ...
            obs_entry = pun.Observables._get_obs_entry(oname)
            valid_kin_vars = [kv for kv in obs_entry.kinematic_variables()]
            pun.info('   plotting pun observable "{}"'.format(oname))
            ...
            if 'parameters' in item and 'parameters-from-file' in item:
                pun.warn('    overriding values read from \'parameters-from-file\' with explicit values in \'parameters\'')

            if 'parameters-from-file' in item and type(item['parameters-from-file']) is str:
                pun.warn('    overriding parameters from file')
            ...
            if item['variable'] in valid_kin_vars:
                ...
                try:
                    qn = pun.QualifiedName(item['variable'])
            ...
            # create observable
            observable = pun.Observable.make(oname, parameters, kinematics, options)
    ...
    class Uncertainty(BasePlot):
         ...
         * ``data-file`` (*str*, path to an existing data file of type *pun.data.Prediction*) -- The path to
           a data file that was generated with the ``pun-analysis`` command-line client.
        ...
        .. code-block::
           ...
           analysis = ... # pun.Analysis object as discussed in the example notebook `predictions.ipynb`
           mu_q2values = numpy.unique(numpy.concatenate((numpy.linspace(0.02,1.00,20), numpy.linspace(1.00,11.60,20))))
           mu_obs      = [pun.Observable.make('B->Dlnu::dBR/dq2', prior.parameters,
                                              pun.Kinematics(q2=q2), pun.Options({'form-factors':'BSZ2015','l':'mu'}))
        ...
        def __init__(self, plotter, item):
            ...
            if 'data' in item and ('data-file' in item or 'hdf5-file' in item):
                pun.warn('   both data and one of data-file or hdf5-file specified; assuming interactive use is intended')
            ...
            if 'data' in item:
                ...
                pun.info('   plotting uncertainty propagation from "{}"'.format(dfname))
                df = pun.data.Prediction(dfname)
                _xvalues = []
                ...
                pun.info('   plotting uncertainty propagation from HDF5 file "{}"'.format(h5fname))
                uncfile = pun.data.UncertaintyDataFile(h5fname)
    ...
    class UncertaintyBinned(BasePlot):
        ...
        def __init__(self, plotter, item):
            ...
            if 'data' in item:
                ...
                h5fname = item['hdf5-file']
                pun.info('   plotting uncertainty propagation from file "{}"'.format(h5fname))
    ...
    class Constraint(BasePlot):
        """Plots constraints from the pun library of experimental and theoretical likelihoods"""
         ...
         * ``constraints`` (:class:`QualifiedName <pun.QualifiedName>` or iterable thereof) -- The name or the list of names of the constraints
           that will be plotted. Must identify at least one of the constraints known to pun; see `the complete list of constraints <../constraints.html>`_.
         * ``variable`` (*str*) -- The name of the kinematic variable to which the x axis will be mapped.

        When plotting multivariate constraints, the following key is also mandatory:

         * ``observable`` (:class:`QualifiedName <pun.QualifiedName>`) -- The name of the observable whose constraints will be plotted.
           Must identify one of the observables known to pun; see `the complete list of observables <../observables.html>`_.
    ...
    class ConstraintOverview(BasePlot):
        """Plots overview of several constraints from the pun library of experimental and theoretical likelihoods"""
        def __init__(self, plotter, item):
            import yaml
        ...
        def plot(self):
            ...
            for constraint in self.constraints:
                if constraint['type'] == 'Gaussian':
                    sigma_hi = np.sqrt(float(constraint['sigma-stat']['hi'])**2 + float(constraint['sigma-sys']['hi'])**2) / width
                    sigma_lo = np.sqrt(float(constraint['sigma-stat']['lo'])**2 + float(constraint['sigma-sys']['lo'])**2) / width
                    latex = '$' + pun.Observables()[constraint['observable']].latex() + '$'
                    xvalues.append(idx)
                    ...
                    for i in range(0, dim):
                        latex = '$' + pun.Observables()[observables[i]].latex() + '$'
    ...
    class Contours2D(BasePlot):
        ...
        def plot(self):
            ...
            h5fname = item['hdf5-file']
            pun.info('   plotting 2D contours from file "{}"'.format(h5fname))
            datafile = pun.data.load_data_file(h5fname)
    ...
    class KernelDensityEstimate1D(BasePlot):
        ...
        .. code-block::
           ...
           analysis = ... # pun.Analysis object as discussed in the example notebook `inference.ipynb`
        ...
        def __init__(self, plotter, item):
            ...
            if 'data' in item and 'hdf5-file' in item:
                pun.warn('   both data and hdf5-file specified; assuming interactive use is intended')
            ...
            if 'data' in item:
                ...
                h5fname = item['hdf5-file']
                pun.info('   plotting histogram from file "{}"'.format(h5fname))
                datafile = pun.data.load_data_file(h5fname)
    ...
    class KernelDensityEstimate2D(BasePlot):
        ...
        def __init__(self, plotter, item):
            ...
            if 'data' in item:
                ...
                h5fname = item['hdf5-file']
                pun.info('   plotting 2D KDE from file "{}"'.format(h5fname))
                datafile = pun.data.load_data_file(h5fname)
    ...
    class SignalPDF(BasePlot):
        """Plots a single pun signal PDF w/o uncertainties as a function of one kinemtic variable"""
        def __init__(self, plotter, item):
            super().__init__(plotter, item)
        ...
        def plot(self):
            item = self.item
            pname = item['pdf']
            pun.info('   plotting pun PDF "{}"'.format(pname))
            ...
            # create observable
            pdf  = pun.SignalPDF.make(pname, parameters, kinematics, options)
    ...
    class Watermark(BasePlot):
        """Inserts an pun watermark into the plots"""
        def __init__(self, plotter, item):
            super().__init__(plotter, item)
        ...
        def plot(self):
            ...
            ax = plt.gca()
            color = 'OrangeRed'
            version = 'v{version}'.format(version=pun.__version__)
            if 'preliminary' in item and item['preliminary']:
                color = 'red'
                version = 'Preliminary'
            ax.text(x, y, r'\textsf{{\textbf{{pun {version}}}}}'.format(version=version),
    ...
    def plot_contents(self):
        ...
        for item in contents:
            ...
            if 'name' not in item:
                name = None
                pun.debug('plotting anonymous contents of type \'{}\''.format(item_type))
            else:
                name = item['name']
                pun.info('plotting "{}"'.format(name))
