...
#include "pun/statistics/goodness-of-fit.hh"
#include "pun/statistics/log-likelihood.hh"
#include "pun/statistics/log-posterior.hh"
#include "pun/statistics/log-prior.hh"
...
namespace impl
{
    ...
    Kinematics_ctor(tuple args, dict kwargs)
    {
        ...
        if (1 < len(args))
        {
            PyErr_SetString(PyExc_TypeError, "pun.Kinematics expects exactly one argument, or keyword arguments, but not both");
            return object();
        }
    ...
    Options_ctor(tuple args, dict kwargs)
    {
        ...
        if (1 < len(args))
        {
            PyErr_SetString(PyExc_TypeError, "pun.Options expects exactly one argument, or keyword arguments, but not both");
            return object();
        }
...
BOOST_PYTHON_MODULE(_pun)
{
    ...
    // native pun logging: provide functions and enum type
    ...
    class_<QualifiedName>("QualifiedName", R"(
            ...
            pun uses qualified names when naming any observable or constraint. The
    ...
    class_<Kinematics>("Kinematics", R"(
            Represents the set of kinematic variables relevant to an :class:`observable <pun.Obserable>`.
            ...
            .. code-block::
               ...
               k = pun.Kinematics(q2=0.4, k2=0.0)                      # default keyword arguments
               k = pun.Kinematics({'q2': 0.4, 'cos(theta_l)': -1.0})   # use a dictionary if variable names are not
    ...
    class_<Options>("Options", R"(
            ...
            .. code-block::
               ...
               o = pun.Options(model='WET')                   # default keyword arguments
               o = pun.Options({'form-factors': 'BSZ2015'})   # use a dictionary if option keys are not
                                                              # valid python identifiers
        )", no_init)
    ...
    class_<Unit>("Unit", R"(
            ...
            Seven possible entries are currently implemented in pun:
            - Undefined
            - None
            - GeV
            - GeV2
    ...
    class_<LogLikelihood>("LogLikelihood", R"(
            Represents the log(likelihood) of a Bayesian analysis undertaken with the :class:`Analysis <pun.Analysis>` class.
        )", init<Parameters>())
    ...
    class_<pun::ConstraintEntry, boost::noncopyable>("ConstraintEntry", no_init)
    ...
    class_<LogPrior, boost::noncopyable>("LogPrior", R"(
        ...
        .def("Uniform", &LogPrior::Flat, return_value_policy<return_by_value>(), R"(
            ...
            :type parameters: pun.Parameters
    ...
    class_<Observable, bases<ParameterUser, ReferenceUser>, boost::noncopyable>("Observable", R"(
            Represents an observable or pseudo observable known to pun.

            New observable objects are created using the :meth:`make <pun.Observable.make>` static method.
    ...
    class_<Observables>("_Observables")
        ...
        .def("insert", &Observables::insert, R"(
            Insert a new observable to pun by parsing the input string.
    ...
    class_<SignalPDF, boost::noncopyable>("_SignalPDF", R"(
            Represents a probability density function (PDF) for any of the physics signals known to pun.

            New PDF objects are created using the :meth:`make <pun.SignalPDF.make>` static method.
            See also `the complete list of PDFs <../signal-pdfs.html>`_.
    )", no_init)
        .def("make", &SignalPDF::make, return_value_policy<return_by_value>()) // docstring is maintained in python/pun/signal_pdf.py
        .staticmethod("make")
        ...
        .def("normalization", &SignalPDF::normalization, R"(
            ...
            To speed up sampling from the PDF, the :meth:`evaluate <pun.SignalPDF.evaluate>` returns values of an
