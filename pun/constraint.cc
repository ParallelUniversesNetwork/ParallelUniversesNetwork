...
#include <pun/maths/gsl-interface.hh>
...
#include <pun/utils/observable_set.hh>
...
namespace pun
{
    ...
    load_constraint_entries()
    {
        ...
        fs::path base;
        if (std::getenv("pun_TESTS_CONSTRAINTS"))
        {
            std::string envvar = std::string(std::getenv("pun_TESTS_CONSTRAINTS"));
            base = fs::system_complete(envvar);
        }
        else if (std::getenv("pun_HOME"))
        {
            std::string envvar = std::string(std::getenv("pun_HOME"));
            base = fs::system_complete(envvar) / "constraints";
        }
        else
        {
            base = fs::system_complete(pun_DATADIR "/pun/constraints/");
        }
