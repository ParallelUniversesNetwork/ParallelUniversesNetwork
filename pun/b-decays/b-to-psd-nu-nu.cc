...
#include <pun/form-factors/form-factors.hh>
...
#include <pun/utils/quantum-numbers.hh>
...
namespace pun
{
    ...
    struct Implementation<BToPseudoscalarDineutrino>
    {
        ...
        double differential_decay_width(const double & q2) const
        {
            ...
            const double lambda = pun::lambda(m_B2, m_P2, q2), sqrt_lambda = std::sqrt(lambda);
            ...
            const double contr_vector = lambda / 24.0 * f_p * f_p * std::norm(wc.cL() +  wc.cR());
