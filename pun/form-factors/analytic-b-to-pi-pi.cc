...
#include <pun/form-factors/pi-lcdas.hh>
...
namespace pun
{
    ...
    struct Implementation<AnalyticFormFactorBToPiPiBFvD2016>
    {
        ...
        static double gsl_adapter(double * parameters, size_t dim, void * _imp)
        {
            ...
            auto lambda = pun::lambda(q2, k2, imp->m_B() * imp->m_B());
            auto lambda_pi = pun::lambda(q2, 0.135 * 0.135, imp->m_B() * imp->m_B());
            if (lambda <= 0)
                return 0.0;
        ...
        {
            return pun::lambda(m_B() * m_B(), 0.135 * 0.135, q2) / pow(b_to_pi_ff->f_p(q2), 2);
        }
        ...
        {
            const double m_B = this->m_B(), m_B2 = m_B * m_B;
            const double lambda = pun::lambda(m_B2, q2, k2);
    ...
    struct Implementation<AnalyticFormFactorBToPiPiFvDV2018>
    {
        ...
        {
            return pun::lambda(q2, k2, m_B() * m_B());
        }
