            ...
            public NonlocalFormFactor<nff::PToP>
        {
            public:
                ...
                inline complex<double> phi(const double & q2, const unsigned phiParam[4]) const
                {
                    ...
                    const auto   z     = pun::nff_utils::z(q2, 4.0 * pow(m_D0, 2), s_0);
                                                      ...
                                                      const complex<double> & alpha_2) const
                {
                    ...
                    const auto zBP     = pun::nff_utils::z(pow(m_B + m_P, 2), s_p, s_0);
                    const auto z_Jpsi  = pun::nff_utils::z(m_Jpsi2,           s_p, s_0);
                    const auto z_psi2S = pun::nff_utils::z(m_psi2S2,          s_p, s_0);

                    const complex<double> dzds = -pow(s_p - s_0, 0.5) * pow(s_p - m_Jpsi2, -0.5) * pow(pow(s_p - m_Jpsi2, 0.5) + pow(s_p - s_0, 0.5), -2);

                    return pun::nff_utils::PGvDV2020(z_Jpsi, zBP, alpha_0, alpha_1, alpha_2) / phi(m_Jpsi2, phiParam) *
                                                       ...
                                                       const complex<double> & alpha_2) const
                {
                    ...
                    return pun::nff_utils::PGvDV2020(z_psi2S, zBP, alpha_0, alpha_1, alpha_2) / phi(m_psi2S2, phiParam) *
                ...
                virtual complex<double> H_plus(const double & q2) const
                {
                    ...
                    const double s_0   = this->t_0();
                    const double s_p   = 4.0 * pow(m_D0, 2);
                    const auto z       = pun::nff_utils::z(q2,              s_p, s_0);
                    const auto zBP     = pun::nff_utils::z(pow(m_B+m_P, 2), s_p, s_0);
                    const auto z_Jpsi  = pun::nff_utils::z(pow(m_Jpsi, 2),  s_p, s_0);
                    const auto z_psi2S = pun::nff_utils::z(pow(m_psi2S, 2), s_p, s_0);

                    const complex<double> blaschke_factor = pun::nff_utils::blaschke_cc(z, z_Jpsi, z_psi2S);

                    const unsigned phiParam[4] = {3, 3, 2, 2};

                    return pun::nff_utils::PGvDV2020(z, zBP, alpha_0, alpha_1, alpha_2) / phi(q2, phiParam) / blaschke_factor;
                }
                ...
                virtual complex<double> Hhat_plus(const double & q2) const
                {
                    ...
                    return pun::nff_utils::PGvDV2020(z, zBP, alpha_0, alpha_1, alpha_2);
                }
                ...
                virtual Diagnostics diagnostics() const
                {
                    ...
                    const double s_0   = this->t_0();
                    const auto z1 = pun::nff_utils::z(1.0, 4.0 * pow(m_D0, 2), s_0);

                    results.add({ real(pun::nff_utils::PGvDV2020(z1, complex<double>(0.6,0.8), 2.0, 3.0, 4.0)),
                                "Re{PGvDV2020(q2 = 1.0, sXY = 0.6+0.8i, 2.0, 3.0, 4.0)}" });
                    results.add({ imag(pun::nff_utils::PGvDV2020(z1, complex<double>(0.6,0.8), 2.0, 3.0, 4.0)),
            ...
            public NonlocalFormFactor<nff::PToP>
        {
            public:
                                                      ...
                                                      const complex<double> & alpha_2) const
                {
                    ...
                    return pun::nff_utils::P(z_Jpsi, alpha_0, alpha_1, alpha_2) / phi(m_Jpsi2, phiParam) *
                            (1 - norm(z_Jpsi)) * (1. - z_Jpsi * std::conj(z_psi2S)) / (z_Jpsi - z_psi2S) / dzds;
                }
                                                       ...
                                                       const complex<double> & alpha_2) const
                {
                    ...
                    return pun::nff_utils::P(z_psi2S, alpha_0, alpha_1, alpha_2) / phi(m_psi2S2, phiParam) *
                            (1 - norm(z_psi2S)) * (1. - z_psi2S * std::conj(z_Jpsi)) / (z_psi2S - z_Jpsi) / dzds;
                }
                ...
                virtual complex<double> H_plus(const double & q2) const
                {
                    ...
                    return pun::nff_utils::P(z, alpha_0, alpha_1, alpha_2) / phi(q2, phiParam) / blaschke_factor;
                }
                ...
                virtual complex<double> Hhat_plus(const double & q2) const
                {
                    ...
                    return pun::nff_utils::P(z, alpha_0, alpha_1, alpha_2);
                }
                ...
                virtual Diagnostics diagnostics() const
                {
                    ...
                    results.add({ real(pun::nff_utils::P(z1, 2.0, 3.0, 4.0)), "Re{P(q2 = 1.0, 2.0, 3.0, 4.0)}" });
                    results.add({ imag(pun::nff_utils::P(z1, 2.0, 3.0, 4.0)), "Im{P(q2 = 1.0, 2.0, 3.0, 4.0)}" });
                    results.add({ real(pun::nff_utils::P(z1, complex<double>(2.0,5.0), complex<double>(3.0,6.0), complex<double>(4.0,7.0))),
                                "Re{P(q2 = 1.0, (2.0,5.0), (3.0,6.0), (4.0,7.0))}" });
                    results.add({ imag(pun::nff_utils::P(z1, complex<double>(2.0,5.0), complex<double>(3.0,6.0), complex<double>(4.0,7.0))),
