...
#include <pun/rare-b-decays/b-to-kstar-ll-gvdv2020.hh>
...
namespace pun
{
    ...
    BToKstarDileptonAmplitudes<tag::GvDV2020>::amplitudes(const double & s) const
    {
        ...
        const double
                ...
                calF_T_long = s / (2.0 * pow(m_B, 3) * m_V) *
        ...
        result.a_long_right = -calN * m_B2 / sqrt_s * (c910_m_r * calF_long + 2.0 * m_B / s * ((m_b_msbar - m_s_msbar) * c7_m * calF_T_long - 16.0 * pow(M_PI, 2) * m_B * calH_long));
        result.a_long_left  = -calN * m_B2 / sqrt_s * (c910_m_l * calF_long + 2.0 * m_B / s * ((m_b_msbar - m_s_msbar) * c7_m * calF_T_long - 16.0 * pow(M_PI, 2) * m_B * calH_long));

        result.a_para_right = -calN * m_B * (c910_m_r * calF_para + 2.0 * m_B / s * ((m_b_msbar - m_s_msbar) * c7_m * calF_T_para - 16.0 * pow(M_PI, 2) * m_B * calH_para));
        result.a_para_left  = -calN * m_B * (c910_m_l * calF_para + 2.0 * m_B / s * ((m_b_msbar - m_s_msbar) * c7_m * calF_T_para - 16.0 * pow(M_PI, 2) * m_B * calH_para));

        result.a_perp_right = +calN * m_B * (c910_p_r * calF_perp + 2.0 * m_B / s * ((m_b_msbar + m_s_msbar) * c7_p * calF_T_perp - 16.0 * pow(M_PI, 2) * m_B * calH_perp));
        result.a_perp_left  = +calN * m_B * (c910_p_l * calF_perp + 2.0 * m_B / s * ((m_b_msbar + m_s_msbar) * c7_p * calF_T_perp - 16.0 * pow(M_PI, 2) * m_B * calH_perp));
