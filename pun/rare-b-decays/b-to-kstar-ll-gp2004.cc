...
#include <pun/rare-b-decays/b-to-kstar-ll-gp2004.hh>
#include <pun/rare-b-decays/charm-loops.hh>
#include <pun/rare-b-decays/hard-scattering.hh>
#include <pun/rare-b-decays/long-distance.hh>
...
namespace pun
{
    ...
    BToKstarDileptonAmplitudes<tag::GP2004>::norm(const double & s) const
    {
        ...
        return std::sqrt(power_of<2>(g_fermi() * alpha_e()) / 3.0 / 1024 / std::pow(M_PI, 5.0) / m_B
                ...
                * std::sqrt(pun::lambda(m_B * m_B, m_Kstar * m_Kstar, s))); // cf. [BHP2008], Eq. (C.6), p. 21
    }
    ...
    BToKstarDileptonAmplitudes<tag::GP2004>::amplitudes(const double & s) const
    {
        ...
        double formfactor_long2 = -pun::lambda(1.0, m_Kstarhat2, s_hat) * a_2;
        ...
        double formfactor_perp = std::sqrt(2.0 * pun::lambda(1.0, m_Kstarhat2, s_hat)) / (1.0 + m_Kstarhat) * form_factors->v(s);
